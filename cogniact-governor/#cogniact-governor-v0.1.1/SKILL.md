...
Use `scripts/selfcheck.py` or `scripts/state.py` for package audit and done-gate evidence on standard or complex skill-maintenance tasks. `selfcheck.py audit-package .` verifies package structure, module links, required rules, script syntax, and the mechanical state smoke path.
