...
## Audit and redundancy discipline

Treat every unit of effort, token, file read, search, edit, tool call, and user attention as a cost. Prefer value per unit effort over ceremony. Do not re-read, re-run, or restate work when fresh evidence already satisfies the stop rule; reuse verified state, then do only the narrowest missing check.

Use `scripts/selfcheck.py` or `scripts/state.py` for package audit and done-gate evidence on standard or complex skill-maintenance tasks. Those paths are relative to the skill package root, so set `SKILL_DIR` to that root (the directory containing this SKILL.md) and invoke `"$SKILL_DIR/scripts/..."` from any working directory. `python3 "$SKILL_DIR/scripts/selfcheck.py" audit-package "$SKILL_DIR"` verifies package structure, module links, required rules, script syntax, and the mechanical state smoke path.
...
Use `state.py` for standard or complex tasks that are easy to derail, span many steps, or require a durable done gate. Trivial tasks normally do not need a state file.

Point `SKILL_DIR` at this skill's package root (the directory containing this SKILL.md) so the commands work from any cwd:

```sh
SKILL_DIR="<this skill's package root>"   # the directory containing this SKILL.md
python3 "$SKILL_DIR/scripts/state.py" init "<goal>" --tier standard
python3 "$SKILL_DIR/scripts/state.py" add "<observable criterion in the user's terms>"
python3 "$SKILL_DIR/scripts/state.py" phase plan
python3 "$SKILL_DIR/scripts/state.py" phase execute
python3 "$SKILL_DIR/scripts/state.py" phase verify
python3 "$SKILL_DIR/scripts/state.py" meet C1 "ran pytest tests/example.py -> 3 passed, 0 failed"
python3 "$SKILL_DIR/scripts/state.py" phase report
python3 "$SKILL_DIR/scripts/state.py" check
python3 "$SKILL_DIR/scripts/state.py" done
```
