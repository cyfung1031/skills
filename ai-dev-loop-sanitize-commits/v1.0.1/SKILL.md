...
v1.0.1 requirement: do not create branches and do not switch branches while using this skill. The script rewrites by building commit trees and updating the current branch ref in place; it prints the original HEAD for recovery through the reflog.
...
- Do not create backup branches, temporary branches, or switch branches. In v1.0.1 the script must keep the current branch checked out and use `git update-ref` only after the new commit chain is built.
