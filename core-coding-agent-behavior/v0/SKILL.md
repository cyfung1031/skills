...
description: Core always-on coding-agent behavior bundle — task execution, code editing, verification, communication, context management, and persistent memory. Load for every coding/agentic session; combined variant of the six individual ai-skills.
---

# Core Coding-Agent Behavior (combined bundle)

This file is GENERATED by scripts/build-core.sh from the six individual skills — edit those, then regenerate. Precedence: subordinate to system/developer/platform/sandbox/tool/repository and explicit user instructions.
...
3. **Verify.** Run the tests, the build, or the command that proves the change works (see verification skill).
...
- For non-trivial tasks, form a short plan before editing: which files, in what order, how you'll verify. Keep it to a few lines; if the task spans many steps, write it as a checklist in the `task-state.md` file described by the `context-window-management` skill.
...
- Don't redo work: if you already established a fact this session, use it.
...
2. **Tests pass** — run the *narrowest* relevant test first (fast feedback), then the broader suite if the change is risky.
...
- **Don't re-read** files you already have in context, and don't re-establish facts the conversation already settled.
...
Never commit it. This is disposable task scratch, distinct from long-term memory (see the `persistent-memory` skill — durable facts go there, in a user-level directory). You (the agent) delete it when the task ships, and mention the deletion in your summary.
...
- **Consent:** persist memory only where the host platform permits persistent storage. In a new environment, **ask before the first write** — unless the platform provides an explicit, user-approved memory mechanism (a designated memory directory or feature the user has already opted into), in which case announcing the location in one sentence is enough. Never silently create persistent storage outside the workspace.
...
Task-scratch state is NOT memory — see the `context-window-management` skill's `task-state.md`, which lives with the project and is deleted when the task ships.
