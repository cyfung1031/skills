...
This harness protects the rule that the loop follows durable written records and repository files, not conversation memory. Use it for release validation and regression review when changing `SKILL.md`, `modules/LOOP-HARNESS.md`, scripts, installer templates, examples, validators, or status vocabulary.
...
| Missing first record shape | `.ai-dev-loop/` has no role records | Load `modules/LOOP-HARNESS.md` templates before first status/R/K write | Validator checks exact template-loading line |
