...
REQUIRED_FILES = [
    ...
    "SKILL.md", "SCORING-EVIDENCE-SCHEMA.json", "PACKAGE-MANIFEST.json", "VALIDATION-RESULTS.md",
    "modules/LOOP-HARNESS.md", "modules/MODE-HARNESS.md", "modules/BENCHMARK-HARNESS.md", "modules/VALIDATION.md", "modules/RESTRICTION-HARNESS.md",
...
SKILL_PHRASES = [
    ...
    "Always-load is this compact `SKILL.md`",
    "scripts/check-ai-dev-loop-state.py/check-ai-dev-loop-flow.py output when available",
    "targeted module only when needed",
    "Compress prose, not obligations",
    "must not remove required gates",
    "When creating the first R/K/status record, or when record shape is uncertain, load `modules/LOOP-HARNESS.md` templates before writing.",
    ...
    "K owns affected artifacts R did not name",
...
WHOLE_CHANGE_DOCS = {
    ...
    "modules/LOOP-HARNESS.md": ["Whole-change impact scan", "not a complete mechanical task list", "did not explicitly list"],
}
...
def validate_release_templates(root: Path, errors: list[str]) -> None:
    ...
    fc = root / 'modules/BENCHMARK-HARNESS.md'
    if fc.is_file():
        text = read(fc)
        for phrase in ['fixture_id', 'oracle', 'format_contract', 'pass_fail_rule', 'Anti-leakage rule']:
            if phrase not in text:
                add(errors, f"modules/BENCHMARK-HARNESS.md missing phrase: {phrase}")
    mt = root / 'modules/MODE-HARNESS.md'
    if mt.is_file():
        text = read(mt)
        for phrase in ['Minimal mode', 'Release mode', 'Degraded mode', 'validation-tier honesty']:
            if phrase not in text:
                add(errors, f"modules/MODE-HARNESS.md missing phrase: {phrase}")

def validate_restriction_harness(root: Path, errors: list[str]) -> None:
    path = root / 'modules/RESTRICTION-HARNESS.md'
    if not path.is_file():
        return
    text = read(path)
    for phrase in [
        ...
        if phrase not in text:
            add(errors, f"modules/RESTRICTION-HARNESS.md missing phrase: {phrase}")
...
def main() -> int:
    ...
    ref = root / 'modules/LOOP-HARNESS.md'
    if ref.exists():
        rt = read(ref)
        for phrase in REFERENCE_PHRASES:
            if phrase not in rt: add(errors, f"modules/LOOP-HARNESS.md missing section phrase: {phrase}")
        for phrase in ['Token budget and loading order', 'Token savings may compress wording', 'must not remove required gates']:
            if phrase not in rt: add(errors, f"modules/LOOP-HARNESS.md missing token-control phrase: {phrase}")
    ...
    for p in list(root.glob('*.md')) + list((root / 'modules').glob('*.md')):
        text = read(p)
    ...
    if installer.exists():
        with tempfile.TemporaryDirectory(prefix='ai-dev-loop-standalone-') as td:
            ...
            for rel in ['.ai-dev-loop/SKILL.md','.ai-dev-loop/modules/LOOP-HARNESS.md','.ai-dev-loop/status.md','.ai-dev-loop/reviews','.ai-dev-loop/responses','.ai-dev-loop/context','.ai-dev-loop/decisions','.ai-dev-loop/scripts/check-ai-dev-loop-state.py','.ai-dev-loop/scripts/check-ai-dev-loop-flow.py']:
