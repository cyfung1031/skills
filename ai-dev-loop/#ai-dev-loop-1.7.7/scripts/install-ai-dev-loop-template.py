...
- `modules/`: runtime markdown loaded only when needed.
...
def install_template(package_root: Path, target: Path) -> None:
    ...
    if not modules_dir.is_dir():
        raise FileNotFoundError(f'package modules directory not found: {modules_dir}')
    ...
    try:
        ...
        shutil.copytree(modules_dir, target / 'modules', dirs_exist_ok=True)
...
def main() -> int:
    ...
    if not (package_root / 'SKILL.md').is_file():
        ...
        return 1
    if not (package_root / 'modules').is_dir():
        print(f'error: package modules directory not found under: {package_root}', file=sys.stderr)
        return 1
