...
- Do not claim exact token savings, remaining window, truncation risk, test coverage, or completion unless supported by visible telemetry or validation.
...
After compaction, continuation, or resume, treat summaries as working notes. Re-verify load-bearing facts before acting on them: file existence, current branch, changed files, failing tests, external state, and whether the latest user request supersedes earlier work.
